...
test_result = pd.DataFrame(columns = ['filename', 'sub_id', 'visit', 'asd_longitudinal', 'pred_score'])
...
for subfold in range(10):
    ...
    """
    input_features_minus_weight = ['asd_longitudinal','visit', 'gender',
    ...
    """
    #age_idx_train = train.dropna(inplace=True)

    #idx_age_test = test.dropna(inplace=True)

    X_train = train[input_features_minus_weight].values
    X_test = test[input_features_minus_weight].values

    y_train_sarb = train.loc[:, 'ados_sarb_total'].values
    y_test_sarb = test.loc[:, 'ados_sarb_total'].values
    ...
    for i in range(1):
        ...
        if model_type == 'deep':
            ...
            model.save('hj_prev/hj_{}_ASD_clasnn_subfold{}_{}.h5'.format(version, subfold, i))
            ...
            mcp_save = ModelCheckpoint('.mdl_wts.hdf5', save_best_only=True, monitor='val_loss', mode='min')
            reduce_lr_loss = ReduceLROnPlateau(monitor='val_loss', factor=0.01, patience=100, verbose=1, epsilon=1e-4,
                                               mode='min')

            history = model.fit(X_train, y_train.astype(int), epochs=50, batch_size=64, validation_data=(X_test, y_test),
                                callbacks=[earlyStopping, mcp_save, reduce_lr_loss], verbose=0)
            # history = model.fit(X_train, y_train2, epochs=1000
            #                    , batch_size=64, validation_data = (X_test,y_test2),class_weight=class_weights,verbose=0,
            #                    callbacks = [earlyStopping, mcp_save, reduce_lr_loss])

            model.save('hj_prev/hj_{}_ASD_regnn_subfold{}_{}.h5'.format(version, subfold, i))
            scores = model.predict(X_test)
            predicted_classes = np.argmax(scores, axis=-1)
        else:
            if model_type == 'logreg':
                ...
                model = MLPRegressor(
                solver='lbfgs', alpha=1e-5, activation='relu',

                hidden_layer_sizes=(512,256,64, 2), random_state=42)
            elif model_type == 'RF':
                model = RandomForestClassifier(
                    max_depth=9, n_estimators=10, max_features=1, random_state=42)

            elif model_type == 'xgb':
                model = XGBClassifier()

            model.fit(X_train, y_train.astype(int))
            #model.fit(X_smt, y_smt.astype(int))
        ...
        temp = pd.DataFrame(columns=['filename', 'sub_id', 'visit', 'asd_longitudinal', 'pred_score', 'pred_prob', 'ados_sarb_total'])
        test.reset_index(inplace=True)
        temp.loc[:, ['filename', 'sub_id', 'visit', 'asd_longitudinal', 'ados_sarb_total']] = test.loc[:, ['filename', 'sub_id', 'visit',
                                                                                        'asd_longitudinal', 'ados_sarb_total']]
        # test_result=
        #temp.pred_prob = pd.Series(scores)
        # scores.shape
        #temp.pred_score = pd.Series(scores[:,])
        temp.pred_score = pd.Series(predicted_classes)
        temp.pred_prob = pd.Series(prob)
        test_result = pd.concat([test_result, temp])

        # ROC AUC

        if model_type == 'reg' or model_type == 'linreg':
            pass
        else:
...
test_pivoted = test_result.pivot_table( index=['sub_id'], columns=[ 'visit', 'partner'], values=['pred_score','pred_prob', 'asd_longitudinal', 'ados_sarb_total'], aggfunc='first')

#test_pivoted.columns = col_names

test_pivoted.reset_index(inplace=True)

test_pivoted.to_csv('age_wise_ados_nnclassification.csv', index=None)
print('pause')
