...
Uses the exact fold3 pre-computed splits from standard_filenames/, skipping subfold3,
with a 512×3 Keras MLP + AlphaDropout(0.5), 10-run ensemble per subfold, and
RandomOverSampler → SMOTETomek → MinMaxScaler — matching aggregate_to_predictions.py.
"""
...
FOLD       = 3
SUBFOLDS   = [0, 1, 2, 4, 5, 6, 7, 8, 9]  # skip subfold3
N_RUNS     = 10
EPOCHS     = 1000
BATCH_SIZE = 64
LR         = 0.0009
...
def load_split_filenames(split_file):
    with open(split_file) as f:
        names = [line.strip() + '.csv' for line in f if line.strip()]
    return set(names)


def load_indicator_data(indicator):
...
def run_experiment(indicator):
    ...
    for subfold in SUBFOLDS:
        ...
        # 10-run ensemble: average predicted probabilities
...
if __name__ == '__main__':
    ...
    outjson = 'PATH/TO/FEATURES/ml_features_keras_results.json'
    ...
    outcsv = 'PATH/TO/FEATURES/ml_features_keras_results.csv'
